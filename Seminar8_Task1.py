...
def read_file(file_name):
    with open(file_name, encoding='utf-8') as data:
        f_r = DictReader(data)
        return list(f_r) #вернет список словарей, сколько строк столько элементов в списке
    ...
    while True:
        ...
        if command == 'q':
            ...
            if not exists(file_name):
                print('Файл отсутствует, пожалуйста создайте файл')
                continue
            elif not exists(file_name_1):
                create_file(file_name_1)
            copy_file(file_name, file_name_1)
...
#Прочитать список словарей (read_file) и просто записать его в новый файл используя функцию standart_write
#дополнить функцию main командой c (copy)
#из phone.csv в phone2.csv ссылка на репозиторий или архив @cdi999